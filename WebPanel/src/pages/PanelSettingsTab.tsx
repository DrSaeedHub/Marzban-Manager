...
import { Eye, EyeOff, Download, Copy, RefreshCw, Trash2, Loader2 } from 'lucide-react';
...
export default function PanelSettingsTab() {
  ...
  const { data: certData } = usePanelCertificate(panelId);
  const deleteMutation = useDeletePanel();
  const disconnectMutation = useDisconnectPanel();
  ...
  const certificate = certData?.certificate ?? panel?.certificate;
  ...
  const handleDisconnect = () => {
    if (panelId) {
      disconnectMutation.mutate(panelId, {
        onSuccess: () => setShowDisconnectModal(false),
      });
    }
  };
        ...
        <div className="grid gap-4">
          ...
          <div className="grid grid-cols-3 items-center gap-4">
            ...
            <div className="col-span-2 text-sm text-foreground">
              {panel.last_sync ? new Date(panel.last_sync).toLocaleString() : 'Never'}
            </div>
          </div>
        </div>
      </motion.div>
